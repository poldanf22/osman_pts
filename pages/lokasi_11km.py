...
if uploaded_file is not None:
    ...
    rata_maw = df.iloc[r, len_col-32]
    rata_map = df.iloc[r, len_col-31]
    rata_ind = df.iloc[r, len_col-30]
    rata_eng = df.iloc[r, len_col-29]
    rata_sej = df.iloc[r, len_col-28]
    rata_geo = df.iloc[r, len_col-27]
    rata_eko = df.iloc[r, len_col-25]
    rata_sos = df.iloc[r, len_col-26]
    rata_fis = df.iloc[r, len_col-24]
    rata_kim = df.iloc[r, len_col-23]
    rata_bio = df.iloc[r, len_col-22]
    rata_jml = df.iloc[r, len_col-21]

    # rata-rata nilai standar
    rata_Smaw = df.iloc[t, len_col-17]
    rata_Smap = df.iloc[t, len_col-16]
    rata_Sind = df.iloc[t, len_col-15]
    rata_Seng = df.iloc[t, len_col-14]
    rata_Ssej = df.iloc[t, len_col-13]
    rata_Sgeo = df.iloc[t, len_col-12]
    rata_Seko = df.iloc[t, len_col-11]
    rata_Ssos = df.iloc[t, len_col-10]
    rata_Sfis = df.iloc[t, len_col-9]
    rata_Skim = df.iloc[t, len_col-8]
    rata_Sbio = df.iloc[t, len_col-7]
    rata_Sjml = df.iloc[t, len_col-6]

    # max jumlah benar
    max_maw = df.iloc[t, len_col-32]
    max_map = df.iloc[t, len_col-31]
    max_ind = df.iloc[t, len_col-30]
    max_eng = df.iloc[t, len_col-29]
    max_sej = df.iloc[t, len_col-28]
    max_geo = df.iloc[t, len_col-27]
    max_eko = df.iloc[t, len_col-26]
    max_sos = df.iloc[t, len_col-25]
    max_fis = df.iloc[t, len_col-24]
    max_kim = df.iloc[t, len_col-23]
    max_bio = df.iloc[t, len_col-22]
    max_jml = df.iloc[t, len_col-21]

    # max nilai standar
    max_Smaw = df.iloc[t, len_col-17]
    max_Smap = df.iloc[t, len_col-16]
    max_Sind = df.iloc[t, len_col-15]
    max_Seng = df.iloc[t, len_col-14]
    max_Ssej = df.iloc[t, len_col-13]
    max_Sgeo = df.iloc[t, len_col-12]
    max_Seko = df.iloc[t, len_col-11]
    max_Ssos = df.iloc[t, len_col-10]
    max_Sfis = df.iloc[t, len_col-9]
    max_Skim = df.iloc[t, len_col-8]
    max_Sbio = df.iloc[t, len_col-7]
    max_Sjml = df.iloc[t, len_col-6]

    # min jumlah benar
    min_maw = df.iloc[t, len_col-32]
    min_map = df.iloc[t, len_col-31]
    min_ind = df.iloc[t, len_col-30]
    min_eng = df.iloc[t, len_col-29]
    min_sej = df.iloc[t, len_col-28]
    min_geo = df.iloc[t, len_col-27]
    min_eko = df.iloc[t, len_col-26]
    min_sos = df.iloc[t, len_col-25]
    min_fis = df.iloc[t, len_col-24]
    min_kim = df.iloc[t, len_col-23]
    min_bio = df.iloc[t, len_col-22]
    min_jml = df.iloc[t, len_col-21]

    # min nilai standar
    min_Smaw = df.iloc[t, len_col-17]
    min_Smap = df.iloc[t, len_col-16]
    min_Sind = df.iloc[t, len_col-15]
    min_Seng = df.iloc[t, len_col-14]
    min_Ssej = df.iloc[t, len_col-13]
    min_Sgeo = df.iloc[t, len_col-12]
    min_Seko = df.iloc[t, len_col-11]
    min_Ssos = df.iloc[t, len_col-10]
    min_Sfis = df.iloc[t, len_col-9]
    min_Skim = df.iloc[t, len_col-8]
    min_Sbio = df.iloc[t, len_col-7]
    min_Sjml = df.iloc[t, len_col-6]

    data_jml_benar = {'BIDANG STUDI': ['MATEMATIKA (MAT)', 'B. INDONESIA (IND)', 'B. INGGRIS (ENG)', 'SEJARAH (SEJ)', 'GEOGRAFI (GEO)', 'EKONOMI (EKO)', 'SOSIOLOGI (SOS)', 'FISIKA (FIS)', 'KIMIA (KIM)', 'BIOLOGI(BIO)', 'JUMLAH (JML)'],
    ...
    data_n_standar = {'BIDANG STUDI': ['MATEMATIKA (MAT)', 'B. INDONESIA (IND)', 'B. INGGRIS (ENG)', 'SEJARAH (SEJ)', 'GEOGRAFI (GEO)', 'EKONOMI (EKO)', 'SOSIOLOGI (SOS)', 'FISIKA (FIS)', 'KIMIA (KIM)', 'BIOLOGI(BIO)', 'JUMLAH (JML)'],
    ...
    df = df[['LOKASI', 'RANK LOK.', 'RANK NAS.', 'NOMOR NF', 'NAMA SISWA', 'NAMA SEKOLAH', 'KELAS', 'MAW', 'MAP', 'IND',
             'ENG', 'SEJ', 'GEO', 'EKO', 'SOS', 'FIS', 'KIM', 'BIO', 'JML', 'S_MAT', 'S_IND', 'S_ENG', 'S_SEJ', 'S_GEO', 'S_EKO', 'S_SOS', 'S_FIS', 'S_KIM', 'S_BIO', 'S_JML']]
