...
if uploaded_file is not None:
    ...
    rata_maw = df.iloc[r, len_col-116]
    rata_map = df.iloc[r, len_col-115]
    rata_ind = df.iloc[r, len_col-114]
    rata_eng = df.iloc[r, len_col-113]
    rata_sej = df.iloc[r, len_col-112]
    rata_geo = df.iloc[r, len_col-111]
    rata_eko = df.iloc[r, len_col-110]
    rata_sos = df.iloc[r, len_col-109]
    rata_fis = df.iloc[r, len_col-108]
    rata_kim = df.iloc[r, len_col-107]
    rata_bio = df.iloc[r, len_col-106]
    rata_jml = df.iloc[r, len_col-105]

    # rata-rata nilai standar
    rata_Smaw = df.iloc[t, len_col-101]
    rata_Smap = df.iloc[t, len_col-100]
    rata_Sind = df.iloc[t, len_col-99]
    rata_Seng = df.iloc[t, len_col-98]
    rata_Ssej = df.iloc[t, len_col-97]
    rata_Sgeo = df.iloc[t, len_col-96]
    rata_Seko = df.iloc[t, len_col-95]
    rata_Ssos = df.iloc[t, len_col-94]
    rata_Sfis = df.iloc[t, len_col-93]
    rata_Skim = df.iloc[t, len_col-92]
    rata_Sbio = df.iloc[t, len_col-91]
    rata_Sjml = df.iloc[t, len_col-90]

    # max jumlah benar
    max_maw = df.iloc[t, len_col-116]
    max_map = df.iloc[t, len_col-115]
    max_ind = df.iloc[t, len_col-114]
    max_eng = df.iloc[t, len_col-113]
    max_sej = df.iloc[t, len_col-112]
    max_geo = df.iloc[t, len_col-111]
    max_eko = df.iloc[t, len_col-110]
    max_sos = df.iloc[t, len_col-109]
    max_fis = df.iloc[t, len_col-108]
    max_kim = df.iloc[t, len_col-107]
    max_bio = df.iloc[t, len_col-106]
    max_jml = df.iloc[t, len_col-105]

    # max nilai standar
    max_Smaw = df.iloc[t, len_col-101]
    max_Smap = df.iloc[t, len_col-100]
    max_Sind = df.iloc[t, len_col-99]
    max_Seng = df.iloc[t, len_col-98]
    max_Ssej = df.iloc[t, len_col-97]
    max_Sgeo = df.iloc[t, len_col-96]
    max_Seko = df.iloc[t, len_col-95]
    max_Ssos = df.iloc[t, len_col-94]
    max_Sfis = df.iloc[t, len_col-93]
    max_Skim = df.iloc[t, len_col-92]
    max_Sbio = df.iloc[t, len_col-91]
    max_Sjml = df.iloc[t, len_col-90]

    # min jumlah benar
    min_maw = df.iloc[t, len_col-116]
    min_map = df.iloc[t, len_col-115]
    min_ind = df.iloc[t, len_col-114]
    min_eng = df.iloc[t, len_col-113]
    min_sej = df.iloc[t, len_col-112]
    min_geo = df.iloc[t, len_col-111]
    min_eko = df.iloc[t, len_col-110]
    min_sos = df.iloc[t, len_col-109]
    min_fis = df.iloc[t, len_col-108]
    min_kim = df.iloc[t, len_col-107]
    min_bio = df.iloc[t, len_col-106]
    min_jml = df.iloc[t, len_col-105]

    # min nilai standar
    min_Smaw = df.iloc[t, len_col-101]
    min_Smap = df.iloc[t, len_col-100]
    min_Sind = df.iloc[t, len_col-99]
    min_Seng = df.iloc[t, len_col-98]
    min_Ssej = df.iloc[t, len_col-97]
    min_Sgeo = df.iloc[t, len_col-96]
    min_Seko = df.iloc[t, len_col-95]
    min_Ssos = df.iloc[t, len_col-94]
    min_Sfis = df.iloc[t, len_col-93]
    min_Skim = df.iloc[t, len_col-92]
    min_Sbio = df.iloc[t, len_col-91]
    min_Sjml = df.iloc[t, len_col-90]
